...
class _ArtistDetailPageState extends State<ArtistDetailPage> {
  // final OnAudioQuery _audioQuery = OnAudioQuery(); // Unused
  ...
  Future<List<SongModel>> _load() async {
    ...
    final list = allSongs.where((s) => (s.artist ?? 'Inconnu').trim().toLowerCase() == widget.artistName.trim().toLowerCase()).toList();
    
    // Apply hidden folder filter
    final filtered = cubit.filterSongs(list);
  ...
  // Selection Actions (Duplicates logic from AlbumDetailPage - good for consistency)
  void _playSelected() {
  ...
  Future<void> _shareSelected() async {
    ...
    final xFiles = <XFile>[];
    for (final s in selectedSongs) {
      if ((s.data).isNotEmpty) { // s.data is String path
